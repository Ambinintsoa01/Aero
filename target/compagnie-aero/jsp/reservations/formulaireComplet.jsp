...
<head>
    ...
    <title>Créer Réservation - Compagnie Aérienne</title>
    <style>
...
<body>
    ...
    <div class="main-content">
        ...
        <div class="card">
            ...
            <div class="info">
                💡 Sélectionnez un passager et les vols souhaités
            </div>

            <form method="POST" action="${pageContext.request.contextPath}/reservations">
                <input type="hidden" name="action" value="creation_complet">
                
                <div class="form-group">
                    <label>Passager</label>
                    <select name="passager_id" required>
                        <option value="">-- Sélectionner un passager --</option>
                        <c:forEach var="p" items="${passagers}">
                            <option value="${p.passager_id}">${p.prenom} ${p.nom} (${p.num_passeport})</option>
                        </c:forEach>
                    </select>
                </div>

                <div class="form-group">
                    <label>Instances de Vol (séparées par des virgules)</label>
                    <input type="text" name="instance_ids" placeholder="ex: 1,2,3" required>
                </div>
                ...
                <div class="form-group">
                    <label>Classe</label>
                    <input type="number" name="classe" placeholder="ex: 1" required>
                </div>

                 <div class="form-group">
                    <label>Siège</label>
                    <input type="text" name="siege_assigne" placeholder="ex: 1" required>
                </div>

                <button type="submit" class="btn">Créer Réservation</button>
                <a href="${pageContext.request.contextPath}/reservations" class="btn" style="background: #95a5a6; margin-left: 10px;">Annuler</a>
            </form>
        </div>
    </div>
</body>
